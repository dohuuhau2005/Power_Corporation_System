...
UPDATE hoadon SET thanhToan = 0 WHERE SOHD ='01KN98GS8JFF7MBVJ3BXQ25145';
commit;
delete from hoadon;
...
CREATE TABLE chitiethoadon (
    id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, -- Khóa chính tự tăng
    soHDN VARCHAR2(26),                                     -- Khóa ngoại liên kết với Hóa Đơn
    bac NUMBER,                                             -- Bậc mấy (1, 2, 3...)
    loai NVARCHAR2(50),                                     -- Loại: 'Trong Định Mức' hoặc 'Vượt Định Mức'
    soKW NUMBER,                                            -- Số lượng kW
    donGia NUMBER,                                          -- Đơn giá áp dụng
    thanhTien NUMBER,                                       -- Thành tiền của dòng đó
    FOREIGN KEY (soHDN) REFERENCES hoadon(soHDN)            -- Ràng buộc toàn vẹn
);
-- 1. Tạo bảng Bảng Giá Điện Bậc Thang
CREATE TABLE BangGiaKW (
    bac NUMBER PRIMARY KEY,         -- Bậc 1, 2, 3...
    gioiHan NUMBER,                 -- Số kW tối đa của bậc đó (Để NULL nếu là vô cực)
    giaChuan NUMBER NOT NULL        -- Giá tiền niêm yết
);

-- 2. Đổ data chuẩn của EVN vào bảng (Y chang cái mảng JS của sếp)
INSERT INTO BangGiaKW (bac, gioiHan, giaChuan) VALUES (1, 50, 1484);
INSERT INTO BangGiaKW (bac, gioiHan, giaChuan) VALUES (2, 50, 1533);
INSERT INTO BangGiaKW (bac, gioiHan, giaChuan) VALUES (3, 100, 1786);
INSERT INTO BangGiaKW (bac, gioiHan, giaChuan) VALUES (4, 100, 2242);
INSERT INTO BangGiaKW (bac, gioiHan, giaChuan) VALUES (5, 100, 2503);
INSERT INTO BangGiaKW (bac, gioiHan, giaChuan) VALUES (6, NULL, 2587); -- NULL tương đương Infinity

-- 3. Chốt đơn lưu dữ liệu
COMMIT;
...
CREATE OR REPLACE TRIGGER trg_sync_chitiethoadon
...
BEGIN
    ...
    -- 2. Truy vấn lồng 3 bảng để "đào" ra Mã Chi Nhánh
    BEGIN
        SELECT k.maCN INTO v_maCN 
        FROM hoadon hd
        JOIN hopdong h ON hd.soHD = h.soHD
        JOIN khachhang k ON h.maKH = k.maKH
        WHERE hd.soHDN = v_soHDN;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            v_maCN := NULL;
    END;

    -- 3. Rẽ nhánh đồng bộ (Đẩy ID và toàn bộ data sang chi nhánh)
    IF v_maCN = 'CN1' THEN
        IF INSERTING THEN
            INSERT INTO chitiethoadon@link_to_cn1 
                (id, soHDN, bac, loai, soKW, donGia, thanhTien) 
            VALUES 
                (:NEW.id, :NEW.soHDN, :NEW.bac, :NEW.loai, :NEW.soKW, :NEW.donGia, :NEW.thanhTien);
        ELSIF UPDATING THEN
            UPDATE chitiethoadon@link_to_cn1 
            SET soHDN = :NEW.soHDN, bac = :NEW.bac, loai = :NEW.loai, 
                soKW = :NEW.soKW, donGia = :NEW.donGia, thanhTien = :NEW.thanhTien 
            WHERE id = :OLD.id;
        ELSIF DELETING THEN
            DELETE FROM chitiethoadon@link_to_cn1 WHERE id = :OLD.id;
        END IF;
        
    ELSIF v_maCN = 'CN2' THEN
        IF INSERTING THEN 
            INSERT INTO chitiethoadon@link_to_cn2 
                (id, soHDN, bac, loai, soKW, donGia, thanhTien) 
            VALUES 
                (:NEW.id, :NEW.soHDN, :NEW.bac, :NEW.loai, :NEW.soKW, :NEW.donGia, :NEW.thanhTien);
        ELSIF UPDATING THEN 
            UPDATE chitiethoadon@link_to_cn2 
            SET soHDN = :NEW.soHDN, bac = :NEW.bac, loai = :NEW.loai, 
                soKW = :NEW.soKW, donGia = :NEW.donGia, thanhTien = :NEW.thanhTien 
            WHERE id = :OLD.id;
        ELSIF DELETING THEN 
            DELETE FROM chitiethoadon@link_to_cn2 WHERE id = :OLD.id; 
        END IF;
        
    ELSIF v_maCN = 'CN3' THEN
        IF INSERTING THEN 
            INSERT INTO chitiethoadon@link_to_cn3 
                (id, soHDN, bac, loai, soKW, donGia, thanhTien) 
            VALUES 
                (:NEW.id, :NEW.soHDN, :NEW.bac, :NEW.loai, :NEW.soKW, :NEW.donGia, :NEW.thanhTien);
        ELSIF UPDATING THEN 
            UPDATE chitiethoadon@link_to_cn3 
            SET soHDN = :NEW.soHDN, bac = :NEW.bac, loai = :NEW.loai, 
                soKW = :NEW.soKW, donGia = :NEW.donGia, thanhTien = :NEW.thanhTien 
            WHERE id = :OLD.id;
        ELSIF DELETING THEN 
            DELETE FROM chitiethoadon@link_to_cn3 WHERE id = :OLD.id; 
        END IF;
        
    ELSIF v_maCN = 'CN4' THEN
        IF INSERTING THEN 
            INSERT INTO chitiethoadon@link_to_cn4 
                (id, soHDN, bac, loai, soKW, donGia, thanhTien) 
            VALUES 
                (:NEW.id, :NEW.soHDN, :NEW.bac, :NEW.loai, :NEW.soKW, :NEW.donGia, :NEW.thanhTien);
        ELSIF UPDATING THEN 
            UPDATE chitiethoadon@link_to_cn4 
            SET soHDN = :NEW.soHDN, bac = :NEW.bac, loai = :NEW.loai, 
                soKW = :NEW.soKW, donGia = :NEW.donGia, thanhTien = :NEW.thanhTien 
            WHERE id = :OLD.id;
        ELSIF DELETING THEN 
            DELETE FROM chitiethoadon@link_to_cn4 WHERE id = :OLD.id; 
        END IF;
...
Grant Select on chitiethoadon to R_STAFF;
...
GRANT SELECT ON db_dienlucCN1.chitiethoadon TO NV_101;
...
GRANT SELECT ON db_dienlucCN3.chitiethoadon TO NV_301;
...
GRANT SELECT ON db_dienlucCN2.chitiethoadon TO NV_201;
...
GRANT SELECT ON db_dienlucCN4.chitiethoadon TO NV_401;
...
CREATE OR REPLACE TRIGGER trg_capnhat_tongtien_hoadon
AFTER INSERT OR UPDATE OR DELETE ON chitiethoadon
FOR EACH ROW
BEGIN
    -- 1. TRƯỜNG HỢP: Thêm một dòng chi tiết mới (Bắn từ Node.js xuống)
    IF INSERTING THEN
        UPDATE hoadon 
        -- Dùng NVL để lỡ soTien đang rỗng (NULL) thì nó coi như bằng 0 để cộng tiếp
        SET soTien = NVL(soTien, 0) + :NEW.thanhTien
        WHERE soHDN = :NEW.soHDN;
        
    -- 2. TRƯỜNG HỢP: Ai đó lén sửa giá của 1 dòng chi tiết
    ELSIF UPDATING THEN
        UPDATE hoadon 
        -- Trừ đi tiền cũ, cộng lại tiền mới 
        SET soTien = NVL(soTien, 0) - :OLD.thanhTien + :NEW.thanhTien
        WHERE soHDN = :NEW.soHDN;
        
    -- 3. TRƯỜNG HỢP: Xóa một dòng chi tiết
    ELSIF DELETING THEN
        UPDATE hoadon 
        -- Trừ đi số tiền của dòng bị xóa
        SET soTien = NVL(soTien, 0) - :OLD.thanhTien
        WHERE soHDN = :OLD.soHDN;
    END IF;
END;
/
